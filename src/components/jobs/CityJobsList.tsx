...
/**
 * CityJobsList Component
 * Interactive job listing with filters for city landing pages
 */

import { useState, useEffect } from 'react';
import JobCard from '@/components/jobs/JobCard';
import { Filter, Users, ChevronRight, Briefcase, LayoutGrid, SlidersHorizontal, X } from 'lucide-react';
import { WorkType, Job } from '@/types';
import JobsService from '@/services/jobs.service';
import Button from '@/components/ui/Button';
import Badge from '@/components/ui/Badge';
import Card from '@/components/ui/Card';

interface CityJobsListProps {
  initialJobs: Job[];
  cityName: string;
  categories: WorkType[];
  locale: string;
...
export default function CityJobsList({ 
  ...
  const fetchJobs = async (page: number, reset: boolean = false, activeFilters = filters) => {
    ...
    try {
      const params: any = { 
        page, 
        per_page: 12,
        job_location: cityName
      };

      if (activeFilters.work_type_id) params.work_type_id = activeFilters.work_type_id;
  ...
  const clearFilters = () => {
    const defaultFilters = { work_type_id: '', gender: '' };
    setFilters(defaultFilters);
    setCurrentPage(1);
    fetchJobs(1, true, defaultFilters);
  };

  const hasActiveFilters = filters.work_type_id || filters.gender;
          ...
          <div className="space-y-10">
            ...
            <div>
              <div className="flex items-center gap-2 mb-4 px-1">
                ...
                <h4 className="text-[10px] font-black text-neutral-400 uppercase tracking-[0.2em]">{jDict.categories}</h4>
              </div>
              <select
              ...
              >
                <option value="">{jDict.allCategories}</option>
                ...
                <div className="flex flex-wrap gap-2">
                  {filters.work_type_id && (
                    <Badge variant="brand" className="bg-brand-50 text-brand-700 border-none text-[9px] font-black uppercase py-1">
                      {categories.find(c => String(c.id) === filters.work_type_id)?.name}
                    </Badge>
                  )}
                  {filters.gender && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-10 gap-6">
          <div>
            <h2 className="text-3xl md:text-4xl font-display font-black text-neutral-900 tracking-tight mb-2">
              {filters.work_type_id 
                ? `${categories.find(c => String(c.id) === filters.work_type_id)?.name} Jobs in ${cityName}`
          ...
          <div className="bg-white px-5 py-2.5 rounded-2xl border border-neutral-200 shadow-sm flex items-center gap-3 self-start md:self-auto">
             ...
             <span className="text-neutral-400 font-black text-[10px] uppercase tracking-widest">
                {jDict?.showingResults ? jDict.showingResults.replace('{count}', jobs.length.toString()) : `${jobs.length} results`}
             </span>
          </div>
        </div>
        ...
        {loading ? (
            ...
            {currentPage >= totalPages && (
              <div className="text-center pt-12">
                <div className="inline-flex items-center px-8 py-4 bg-neutral-50 rounded-full text-neutral-400 text-[10px] font-black uppercase tracking-[0.2em] border border-neutral-100">
                   ✨ You've seen all current openings in {cityName}
                </div>
              </div>
            )}
          </>
          ...
          <div className="max-w-2xl mx-auto text-center py-20 bg-white border border-neutral-100 rounded-[3rem] shadow-soft">
            ...
            <p className="text-neutral-600 font-bold opacity-80 mb-10 max-w-md mx-auto">
              We couldn't find any jobs matching your specific filters in {cityName}.
            </p>
            <Button 
              onClick={clearFilters}
